...
<head>
    ...
    @endif
    <link rel="stylesheet" href="{{ url ('/assets/css/aiz-core.css') }}">
...
<body>
    ...
    <script src="{{ url ('/assets/js/aiz-core.js') }}"></script>
    ...
    <script>
		...
		function show_order_track(order_id, waybill)
        {
			console.log(order_id, waybill);
			
			$('#trackOrder-modal-body').html(null);
			...
			if(waybill == undefined){
				let data = '<div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">Waybill not found !!</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"></button></div>';
				$('#trackOrder-modal-body').html(data);
                $('#trackOrder').modal();
                $('.c-preloader').hide();
			} else {
				$.post('{{ route('purchase_history.track') }}', { _token : AIZ.data.csrf, order_id : order_id}, function(data){
					$('#trackOrder-modal-body').html(data);
					$('#trackOrder').modal();
					$('.c-preloader').hide();
				});
		...
		function show_edit_order(order_id, waybill)
        {
			console.log(order_id, waybill);
            $('#editOrder-modal-body').html(null);
			...
			if(waybill == undefined){
				let data = '<div class="modal-header"><h5 class="modal-title" id="exampleModalLabel">Waybill not found !!</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"></button></div>';
				$('#editOrder-modal-body').html(data);
                $('#editOrder').modal();
                $('.c-preloader').hide();
			} else {
				$.post('{{ route('purchase_history.edit') }}', { _token : AIZ.data.csrf, order_id : order_id}, function(data){
					$('#editOrder-modal-body').html(data);
					$('#editOrder').modal();
					$('.c-preloader').hide();
				});
